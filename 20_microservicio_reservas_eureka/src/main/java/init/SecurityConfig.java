...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.inMemoryAuthentication().withUser("user1").password("{noop}user1") // lo de {noop} se pone para no obligar
				...
				.roles("USER").and().withUser("admin").password("{noop}admin").roles("USER", "ADMIN").and()
				.withUser("user2").password("{noop}user2").roles("OPERATOR");
		 ...
		 * .withUser("admin") .password("{bcrypt}"+new
		 * BCryptPasswordEncoder().encode("admin")) .roles("USER", "ADMIN");
		 */
		// la siguiente configuración será para el caso de
		// usuarios en una base de datos
		auth.jdbcAuthentication().dataSource(dataSource())
				.usersByUsernameQuery("select user, pwd, enabled" + " from users where user=?")
				.authoritiesByUsernameQuery("select user, rol " + "from roles where user=?");
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests()
				// y para acceder a la lista de reservas, tendrán que estar autenticados
				.antMatchers(HttpMethod.GET, "/Reservas").hasRole("ADMIN").and().httpBasic();
	}
